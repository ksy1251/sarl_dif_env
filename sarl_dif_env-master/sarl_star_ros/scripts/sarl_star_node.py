...
class RobotAction(object):
    ...
    def update_objects(self, msg):
        ...
        for p in msg.circles:
            ...
            """if p.true_radius <= 0.05:
                radius = 0.05
            elif p.true_radius <= 0.1:
                radius = 0.1
            elif p.true_radius <= 0.15:
                radius = 0.15
            elif p.true_radius <= 0.2:
                radius = 0.2
            elif p.true_radius <= 0.25:
                radius = 0.25
            elif p.true_radius <= 0.3:
                radius = 0.3
            elif p.true_radius <= 0.35:
                radius = 0.35
            elif p.true_radius <= 0.4:
                radius = 0.4
            elif p.true_radius <= 0.45:
                radius = 0.45
            elif p.true_radius <= 0.5:
                radius = 0.5
            elif p.true_radius <= 0.55:
                radius = 0.55
            elif p.true_radius <= 0.6:
                radius = 0.6                
            else:
                radius = p.true_radius
            human = Human(p.center.x, p.center.y, p.velocity.x, p.velocity.y, radius)"""
...
if __name__ == '__main__':
    ...
    model_dir = pack_path + '/CrowdNav/crowd_nav/data/sub/output_11_dgs001/'
